...
1. **"Why is transformation a cosine similarity and not a probability?"** This is the single most likely technical challenge. The answer: a fixed probability would make transformation equivalent to assimilation — two identically parameterized gates in sequence, with no theoretical differentiation. The cosine-similarity gate implements Nooteboom et al.'s (2007) cognitive distance prediction that transfer difficulty is a function of knowledge gap, not a random draw. Have the committee feedback item #2 ("how does learning strategy affect probabilities?") in mind — be ready to walk through Stage 1 acquisition with path_length as the concrete answer to how scenario changes stage transitions.

2. **"The 20-team network is too small for scale-free properties."** This is committee feedback item #1 (from the 2026-03-30 defense). Acknowledge directly: Barabási-Albert networks require approximately 50+ nodes for robust scale-free degree distributions to emerge. The 20-agent BA network in this study is included as a structural contrast condition, not as a demonstration of mature scale-free behavior. The comparison is between WS (high clustering, short path), BA (hub-and-spoke tendency), and ER (random) — the topological differences are meaningful even at small N. Address this proactively in the defense if the committee raises it.

3. **"Why four stages? Why not two?"** Committee feedback item #3. The four-stage Zahra & George (2002) framework is chosen because each stage has a distinct empirical signature and a distinct model mechanism. Collapsing acquisition and assimilation would lose the distance-decay effect (which operates only in Stage 1); collapsing transformation and exploitation would lose the cosine-similarity bottleneck. Two stages cannot separately represent "can the team receive the signal?" and "can the team integrate it with existing knowledge?" — yet these are the two questions the literature identifies as the primary barriers to knowledge transfer (Szulanski 1996).

4. **"What does `signal_decay = 0.8` correspond to empirically?"** This parameter does not have a direct empirical calibration target in the cited literature — it is a design assumption. Be prepared to frame it as a sensitivity parameter: the key finding (that GLOBAL sharing dominates NEIGHBOR sharing) holds across a range of `signal_decay` values, as demonstrated in the sensitivity analysis in Section 3.8. The specific value of 0.8 represents a moderate decay assumption; it is not claimed to be empirically measured.

5. **"The source asymmetry means LOCAL always produces 0% transformation — isn't that a degenerate case?"** No. It is the correct representation of a team that does not share knowledge beyond its own boundary. The 0% transformation rate under LOCAL is the model's prediction of what happens when organizations treat incidents as local events. This is a finding, not a flaw.
